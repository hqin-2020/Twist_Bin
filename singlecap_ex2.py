...
# alpha_c_hat = 0.484       # consumption intercept (estimated) # 0.88
...
while FC_Err > tol and epoch < max_iter:
    ...
    mc = delta*V0**(rho-1)/(A_cap-d_star)**(rho)
    ...
    B_1 = alpha_z_hat-kappa_hat*W1_mat
    ...
    # D += d - phi/2 * d**2 + .01 * (alpha_c_hat + beta_hat *W1_mat - .01/2*(sigma_z[0]**2+sigma_z[1]**2) ) 
    D += .01 * (alpha_c_hat + beta_hat *W1_mat - .01/2*(sigma_z[0]**2+sigma_z[1]**2) ) 
    D -= ell * ( h1**2+hz**2 )/2
